...
配置来源：
- 全局配置：`~/.eevee/settings.json`
- 项目配置：`.eevee/settings.json`
...
class McpManager:
    ...
    def _load_configs(self) -> dict[str, dict]:
        ...
        # 1. 全局配置：~/.eevee/settings.json
        global_path = Path.home() / ".eevee" / "settings.json"
        self._merge_config_file(global_path, merged)

        # 2. 当前项目配置：<cwd>/.eevee/settings.json
        project_path = Path.cwd() / ".eevee" / "settings.json"
