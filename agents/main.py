...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        prog="eevee-agent",
        description="Eevee Agent — a minimal coding agent",
        add_help=False,
    )
    ...
    parser.add_argument("--thinking-level", choices=("none", "low", "medium", "high"),
                        default=None, help="Reasoning effort for OpenAI-compatible models")
    parser.add_argument("--context-window", type=int, default=None,
                        help="Context size Eevee Agent uses for compaction thresholds")
...
def _resolve_context_window(cli_value: int | None) -> int | None:
    raw = cli_value if cli_value is not None else _clean_env(os.environ.get("EEVEE_CONTEXT_WINDOW"))
    if raw is None:
        return None
    try:
        value = int(raw)
    except (TypeError, ValueError) as error:
        raise ValueError("EEVEE_CONTEXT_WINDOW must be a positive integer") from error
    if value <= 0:
        raise ValueError("EEVEE_CONTEXT_WINDOW must be a positive integer")
    return value


def _resolve_reasoning_effort(cli_value: str | None) -> str | None:
    value = cli_value or _clean_env(os.environ.get("OLLAMA_REASONING_EFFORT"))
    if value is None:
        return None
    value = value.lower()
    if value not in {"none", "low", "medium", "high"}:
        raise ValueError("OLLAMA_REASONING_EFFORT must be one of: none, low, medium, high")
    return value
...
async def run_repl(agent: Agent) -> None:
    ...
    async def confirm_fn(message: str) -> bool:
        try:
            answer = await _read_console_line("  Allow? (y/n): ")
    ...
    async def plan_approval_fn(plan_content: str) -> dict:
        ...
        while True:
            try:
                choice = (await _read_console_line("  Enter choice (1-4): ")).strip()
            except EOFError:
            ...
            if choice == "1":
                ...
                try:
                    feedback = (await _read_console_line("  Feedback (what to change): ")).strip()
                except EOFError:
                    feedback = ""
    ...
    while True:
        ...
        if inp == "/skills":
            ...
            if not skills:
                print_info("No skills found. Add skills to .eevee/skills/<name>/SKILL.md")
            else:
...
def main() -> None:
    ...
    if args.help:
        # 自定义帮助文本，展示 Eevee Agent 支持的启动参数和 REPL 内置命令。
        print("""
Usage: eevee-agent [options] [prompt]
...
Options:
  ...
  --thinking-level    OpenAI-compatible reasoning: none, low, medium, or high
  --context-window N  Context size used for Eevee Agent compaction thresholds
...
""")
    ...
    try:
        context_window = _resolve_context_window(args.context_window)
        reasoning_effort = _resolve_reasoning_effort(args.thinking_level)
