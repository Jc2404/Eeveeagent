...
def get_evolution_dir() -> Path:
    configured = os.environ.get("EEVEE_EVOLUTION_AUDIT_DIR", "").strip()
    return Path(configured) if configured else Path.cwd() / ".eevee" / "skill-evolution"
...
def resolve_skill_file(skill_name: str, *, target: str = "active", active_dir: str = "") -> Path | None:
    from .skills import get_project_skill_dir, get_user_skill_dir
    ...
    if target in ("project", "active"):
        found = _find_skill_file_by_name(get_project_skill_dir(), skill_name)
        if found:
            return found

    if target in ("user", "active"):
        found = _find_skill_file_by_name(get_user_skill_dir(), skill_name)
        if found:
            return found
...
def _maybe_prune_stale_skill(skill_name: str, stats: dict[str, Any]) -> bool:
    min_retrieved = _parse_int(os.environ.get("EEVEE_SKILL_USAGE_PRUNE_MIN_RETRIEVED"), 40)
    max_used = _parse_int(os.environ.get("EEVEE_SKILL_USAGE_PRUNE_MAX_USED"), 0)
    source = str(stats.get("source") or "").strip().lower()
    if source != "user" and os.environ.get("EEVEE_SKILL_PRUNE_PROJECT", "").strip().lower() not in {"1", "true", "yes", "on"}:
        return False
