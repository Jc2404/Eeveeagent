...
async def extract_online_skill_candidate(
    *,
    ...
    system = (
        "You are Eevee Agent's online Skill Extractor.\n"
...
async def maintain_online_skill_candidate(
    *,
    ...
    system = (
        "You are Eevee Agent's online Skill Set Manager.\n"
...
async def judge_retrieved_skill_usage(
    *,
    ...
    raw_response = await side_query(system, json.dumps(payload, ensure_ascii=False))
    parsed = _parse_json_object(raw_response)
    raw_judgments = parsed.get("judgments")
    if not isinstance(raw_judgments, list):
        raise ValueError("skill usage judgment returned an empty or invalid response")
    ...
    for hit in hits:
        ...
        if (
            ...
            or not isinstance(raw.get("relevant"), bool)
            or not isinstance(raw.get("used"), bool)
        ):
            raise ValueError(f"skill usage judgment is missing or invalid for: {name}")
        judgments.append(
            {
                "name": name,
                "source": hit.get("source", ""),
