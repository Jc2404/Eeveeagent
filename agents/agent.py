...
from agents.subagent import SUBAGENT_EXCLUDED_TOOLS, get_sub_agent_config
...
class Agent:
    def __init__(self,
                 *,
                 ...
                 enable_skill_retrieval: bool | None=None,
                 enable_skill_evolution: bool | None=None,
                 allow_skill_write: bool | None=None,
        ...
        self.context_window = context_window or _get_context_windows(model)
        ...
        self.enable_skill_retrieval = env_flag("EEVEE_ENABLE_SKILL_RETRIEVAL") if enable_skill_retrieval is None else enable_skill_retrieval
        self.enable_skill_evolution = env_flag("EEVEE_ENABLE_SKILL_EVOLUTION") if enable_skill_evolution is None else enable_skill_evolution
        self.allow_skill_write = env_flag("EEVEE_ALLOW_SKILL_WRITE") if allow_skill_write is None else allow_skill_write
        self.enable_memory = env_flag("EEVEE_ENABLE_MEMORY") if enable_memory is None else enable_memory
        os.environ["EEVEE_ENABLE_SKILL_RETRIEVAL"] = "1" if self.enable_skill_retrieval else "0"
        os.environ["EEVEE_ENABLE_SKILL_EVOLUTION"] = "1" if self.enable_skill_evolution else "0"
        os.environ["EEVEE_ALLOW_SKILL_WRITE"] = "1" if self.allow_skill_write else "0"
        os.environ["EEVEE_ENABLE_MEMORY"] = "1" if self.enable_memory else "0"

        selected_tools = list(custom_tools or tool_definitions)
        if not self.enable_skill_retrieval:
            selected_tools = [tool for tool in selected_tools if tool.get("name") != "skill"]
        if not self.allow_skill_write:
            selected_tools = [tool for tool in selected_tools if tool.get("name") not in {"skill_create", "skill_evolve"}]
        if not self.enable_memory:
            selected_tools = [tool for tool in selected_tools if tool.get("name") != "memory_search"]
        self.tools = selected_tools
        self.max_cost_usd = max_cost_usd
        self.max_turns = max_turns
        self.confirm_fn = confirm_fn
        self._custom_system_prompt = custom_system_prompt
        context_reserve = min(20000, max(1, self.context_window // 10))
        self.effective_window = max(1, self.context_window - context_reserve)
    ...
    async def  chat(self, user_message:str)->None:
        ...
        coro = (
            self._chat_openai(user_message, memory_query=original_user_message)
            if self.use_openai
            else self._chat_anthropic(user_message, memory_query=original_user_message)
        )
        ...
        if not self.is_sub_agent and not self._aborted:
            # Capture turn-local retrieval evidence before scheduling the coroutine.  The
            # next turn resets/replaces ``_last_retrieved_skill_hits`` and a background
            # task may not start until after that happens.
            usage_hits = copy.deepcopy(self._last_retrieved_skill_hits or [])
            self._schedule_background_skill_task(
                self._run_skill_usage_tracking(original_user_message, assistant_text, usage_hits)
            )
   ...
   #执行一次对话，收集本轮模型输出文本，并返回本轮消耗的 token 数
    ...
    def _consume_settled_memory_prefetch(self, memory_prefetch: MemoryPrefetch | None) -> str:
        """消费已经完成的召回结果；任务仍在运行时立即返回。"""
        if memory_prefetch is None or memory_prefetch.consumed or not memory_prefetch.settled:
            return ""
        ...
        try:
            ...
            logging.warning("memory recall failed: %s", error)
            return ""

        memories = self._accept_relevant_memories(memories)
        if not memories:
            return ""
        return _safe_utf8_text(format_memories_for_injection(memories))

    def _accept_relevant_memories(
        self, memories: list[RelevantMemory]
    ) -> list[RelevantMemory]:
        """去重并在 session budget 内登记真正展示给模型的 memories。"""
        accepted: list[RelevantMemory] = []
        for memory in memories:
            if memory.path in self._already_surfaced_memories:
                continue
            remaining = MAX_SESSION_MEMORY_BYTES - self._session_memory_bytes
            if memory.size > remaining:
                continue
    ...
    def _online_evolution_enabled(self) -> bool:
        if not self.enable_skill_evolution or not self.allow_skill_write:
            return False
        raw = os.environ.get("EEVEE_AUTO_SKILL_EVOLUTION", "1").strip().lower()
    ...
    async def _run_online_skill_evolution(self, window: dict[str, Any], *, interactive_confirm: bool = False) -> None:
        ...
        result = await online_ingest(
            ...
            target=os.environ.get("EEVEE_AUTO_SKILL_TARGET", "project"),
        )
        if result.get("ok"):
    ...
    async def _run_skill_usage_tracking(
        ...
        if not hits or not assistant_text.strip():
            return
        side_query = self._build_side_query(max_tokens=1000)
        try:
...
#整理 Anthropic 的历史消息，修正部分角色错误，并丢弃不合法的工具调用消息。
    ...
    def _persist_large_result(self, tool_name: str, result: str) -> str:
        ...
        d = Path.home() / ".eevee-agent" / "tool-results"
        d.mkdir(parents=True, exist_ok=True)
    ...
    async def _execute_tool_call(self, name: str, inp: dict) -> str:
        if name == "skill" and not self.enable_skill_retrieval:
            return "Error: skill retrieval is disabled for this run."
        if name in {"skill_create", "skill_evolve"} and not self.allow_skill_write:
            return "Error: skill writes are disabled for this run."
        ...
        if name == "memory_search":
            return await self._execute_memory_search_tool(inp)
        if name == "skill":
        ...
        if result["context"] == "fork":
            ...
            tools = (
                [
                    ...
                    if t["name"] in result["allowed_tools"]
                    and t["name"] not in SUBAGENT_EXCLUDED_TOOLS
                ]
                #result.get("allowed_tools") - 安全访问
                # 存在key：返回对应的值（可能是 None、[]、["tool1"] 等）
                # 不存在key：返回 None（不会抛异常）
                if result.get("allowed_tools")
                else [t for t in self.tools if t["name"] not in SUBAGENT_EXCLUDED_TOOLS]
            )
    ...
    async def _execute_memory_search_tool(self, inp: dict) -> str:
        if self.is_sub_agent:
            return "Error: memory_search is only available to the main agent."

        query = str(inp.get("query") or "").strip()
        if not query:
            return "Error: query must not be empty."

        if self._session_memory_bytes >= MAX_SESSION_MEMORY_BYTES:
            return "Memory search skipped: the session memory budget is exhausted."

        side_query = self._build_side_query()
        if side_query is None:
            return "Error: memory search is unavailable for the current model backend."
        ...
        accepted = self._accept_relevant_memories(memories)
        return format_memory_search_results(query, accepted)
...
#--------------Anthropic 后端---------------
    async def _chat_anthropic(self, user_message: str, *, memory_query: str | None = None) -> None:
        ...
        # 主 agent 异步预取 memory；未完成时不阻塞主模型请求。
        memory_prefetch: MemoryPrefetch | None = None
        if not self.is_sub_agent and getattr(self, "enable_memory", True):
            sq = self._build_side_query()
            if sq:
                memory_prefetch = start_memory_prefetch(
                    memory_query if memory_query is not None else user_message, sq,
        ...
        while True:
            ...
            injection_text = self._consume_settled_memory_prefetch(memory_prefetch)
            if injection_text:
...
#调用 _do()，如果遇到可重试错误，就由 _with_retry() 负责重试。
    ...
    async def _chat_openai(self, user_message: str, *, memory_query: str | None = None) -> None:
        user_message = _safe_utf8_text(user_message)
    ...
    async def _confirm_dangerous(self, command: str) -> bool:
        ...
        # Keep the event loop available to background skill/memory tasks while the
        # terminal waits for confirmation.
        try:
