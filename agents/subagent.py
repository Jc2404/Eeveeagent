...
另外支持通过 .eevee/agents/*.md 定义用户自定义代理。"""
...
# 自定义代理发现结果的进程内缓存；读取 .eevee/agents/*.md 后会复用，避免每次调用 agent 工具都扫目录。
_cached_custom_agents: dict[str, dict] | None = None
# 只有主 Agent 能创建 sub-agent 或查找长期记忆。
SUBAGENT_EXCLUDED_TOOLS = {"agent", "memory_search"}
...
def _discover_custom_agents() -> dict[str, dict]:
    ...
    _load_agents_from_dir(Path.home() / ".eevee" / "agents", agents)
    # Project-level (higher priority, overwrites)
    _load_agents_from_dir(Path.cwd() / ".eevee" / "agents", agents)
...
def get_sub_agent_config(agent_type: str) -> dict:
    ...
    if custom:
        if custom["allowed_tools"]:
            ...
            tools = [
                ...
                if t["name"] in custom["allowed_tools"]
                and t["name"] not in SUBAGENT_EXCLUDED_TOOLS
            ]
        else:
            # 不允许子代理再调用 agent 工具，避免递归创建子代理导致控制流复杂化。
            tools = [t for t in tool_definitions if t["name"] not in SUBAGENT_EXCLUDED_TOOLS]
    ...
    if agent_type == "explore":
        ...
        return {"system_prompt": GENERAL_PROMPT, "tools": [t for t in tool_definitions if t["name"] not in SUBAGENT_EXCLUDED_TOOLS]}
...
def reset_agent_cache() -> None:
    """清空自定义代理缓存；测试或运行中刷新 .eevee/agents 配置时使用。"""
    global _cached_custom_agents
