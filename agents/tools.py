...
READ_TOOLS = {"read_file", "list_files", "grep_search", "compact_context", "memory_search"}
...
tool_definitions: list[ToolDef] = [
    ...
    {
        "name": "skill",
        "description": "Invoke a registered skill by name. Skills are prompt templates loaded from .eevee/skills/. Returns the skill's resolved prompt to follow.",
        "input_schema": {
            "type": "object",
            "properties": {
    ...
    },
    {
        "name": "memory_search",
        "description": (
            "Search persistent project memories for context relevant to a specific query. "
            "Use when the task depends on earlier user preferences, feedback, decisions, "
            "or project context that is not already available, or when new clues discovered "
            "during tool use suggest a different memory topic. This tool is read-only. "
            "Memories are historical context; verify code-related claims against the current repository."
        ),
        "input_schema": {
            "type": "object",
            "properties": {
                "query": {
                    "type": "string",
                    "minLength": 1,
                    "description": "A concise, standalone description of the historical context, decision, preference, or prior feedback to retrieve.",
                },
            },
            "required": ["query"],
    ...
    {
        "name": "agent",
        "description": "Launch a sub-agent to handle a task autonomously. Sub-agents have isolated context and return their result. Types: 'explore' (read-only), 'plan' (read-only, structured planning), 'general' (full tools), or other registered custom agent names.",
        "input_schema": {
            "type": "object",
            "properties": {
                ...
                "type": {
                    "type": "string",
                    "description": "Agent type: explore, plan, general, or a registered custom agent name. Default: general",
                },
            },
...
def _resolve_tool_path(raw_path: str, *, must_exist: bool = True) -> Path:
    path = Path(raw_path)
    # A write target is allowed not to exist yet. Preserve it exactly so that an
    # absolute path cannot be silently rebased into the current working directory.
    if path.exists() or not path.is_absolute() or not must_exist:
        return path
...
def _write_file(inp:dict) -> str:
    try:
        ...
        return f"Successfully wrote to {path} ({line_count} lines)\n\n{preview}{trunc}"
    except Exception as e:
...
def load_permission_rules() -> dict:
    ...
    user_settings = _load_settings(Path.home() / ".eevee" / "settings.json")
    project_settings = _load_settings(Path.cwd() / ".eevee" / "settings.json")
